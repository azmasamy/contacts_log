...
switch ($_SERVER['REQUEST_METHOD']) {
      ...
      while ($contact_row = mysqli_fetch_assoc($contacts_resutl)) {
        ...
        if($contact_numbers) {
          while ($numbers_row = mysqli_fetch_assoc($contact_numbers)) {
            ...
            $contact_info['numbers'][] = $numbers_row['phone_number'];
            $contact_info['phone_title'][] = $numbers_row['phone_title'];
            $contact_info['default_numbers'][] = $numbers_row['default_num'];
          }
        } else {
          http_response_code(400);
  ...
  case 'PUT': {
    ...
    if ($body) {
      ...
      foreach ($phone_titles as $index => $title) {
        if($title == "") {
          unset($phone_titles[$index]);
        }
      }
      $default_numbers = $body['default_numbers'];
      foreach ($default_numbers as $index => $default) {
        if($default == "") {
      ...
      if(empty($contact_id) || empty($first_name) || empty($last_name) || empty($numbers) || empty($phone_titles) || empty($default_numbers) || count($numbers) != count($phone_titles) || count($numbers) != count($default_numbers)) {
        http_response_code(400);
  ...
  case 'POST': {
    ...
    if ($body) {
      ...
      if(empty($first_name) || empty($last_name) || empty($numbers) || empty($phone_titles) || empty($default_numbers) || count($numbers) != count($phone_titles) || count($numbers) != count($default_numbers)) {
        http_response_code(400);
